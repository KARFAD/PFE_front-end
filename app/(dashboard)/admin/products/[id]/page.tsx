...
const DashboardProductDetails = ({
  ...
  const deleteProduct = async () => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/products/${id}`, requestOptions)
      .then((response) => {
  ...
  const fetchProductData = async () => {
    ...
    const imagesData = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/images/${id}`, {
      cache: "no-store",
    });
        ...
        <div>
          <input
            ...
            onChange={(e) => {
              ...
              if (!files || files.length === 0) return; // 👈 sécurité
